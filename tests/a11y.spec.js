 ...
 *
 * Escaneia cada página com axe-core nas regras WCAG 2.0/2.1/2.2 níveis A e AA.
 * Saída: lista de violações (se houver) e contagem de checks aprovados.
 * Exit code 1 se qualquer violação for encontrada em qualquer página.
 */
...
// Páginas do site a serem auditadas.
const PAGES = ['', 'glossario.html'];

// Tags axe correspondentes ao alvo declarado: WCAG 2.2 AA.
const WCAG_TAGS = [
  'wcag2a',
  'wcag2aa',
  'wcag21a',
...
async function auditPage(page, url) {
  console.log(`\n${BOLD}▶ Auditando ${url}${RESET}`);

  try {
    await page.goto(url, { waitUntil: 'networkidle' });
  } catch (e) {
    console.log(`${RED}Falha ao carregar ${url}: ${e.message}${RESET}`);
  ...
  console.log(
    `${BOLD}Resumo (${url}):${RESET} ${GREEN}${passes.length} checks aprovados${RESET} · ` +
...
(async () => {
  ...
  for (const p of PAGES) {
    const code = await auditPage(page, BASE_URL + p);
